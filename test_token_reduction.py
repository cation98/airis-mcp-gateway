...
比較:
1. Docker MCP Gateway (http://gateway.localhost/sse) - オリジナル
2. FastAPI Proxy (http://gateway.localhost/api/v1/mcp/sse) - Schema Partitioned
"""
...
async def main():
    ...
    proxy_data = await fetch_tools_via_gateway("http://gateway.localhost/api/v1/mcp/sse")
