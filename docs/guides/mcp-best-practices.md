...
> ℹ️ **Sequential Thinking** は npm 版（`sequential-thinking`）に一本化し、Gatewayセクションで管理します。
...
| **sequential-thinking** | ✅ 有効 | 🟢 **必須** | `@modelcontextprotocol/server-sequential-thinking`（多段階推論API） |
...
**理由**:
1. ✅ **Sequential-Thinking/Time/Fetch** すべて有効化済み
...
```yaml
有効化条件:
  - 常時有効（npm版 `sequential-thinking` をGatewayで起動）
  - 複雑なタスクでは自動起動
  - トークンコストは許容範囲
...
```json
{
  ...
  "mcpServers": {
    ...
    "__builtin": "time, fetch, git, memory",
